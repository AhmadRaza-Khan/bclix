...
const VideoCarousel = () => {
  ...
  return (
    <div className={`relative h-screen ${isDropdownOpen ? 'w-1/2 h-screen mask mask-parallelogram-4' : 'w-full'}`}>
      {/* Video Background */}
      <video
        className={`absolute inset-0 w-full h-full object-cover transition-transform duration-500 ${isDropdownOpen ? 'own-transform' : ''}`}
        autoPlay
        loop
        muted
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center text-white z-10">
        ...
        <p className={`font-bold mb-4 ${isDropdownOpen ? 'text-xs' : 'text-lg'}`}>
